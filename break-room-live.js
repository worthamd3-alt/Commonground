    ...
    Se = async S => {
      ...
      try {
        let {
          data: ppl,
          error: pplErr
        } = await F.from("profiles").select("id").eq("active", !0);
        if (pplErr) return "Couldn't send: " + pplErr.message;
        let to = (ppl || []).map(row => row.id).filter(id => id !== (e && e.id));
        ...
        return n ? "Notified " + n + (n === 1 ? " device" : " devices") : res && res.note || "Nothing was sent"
      } catch (ex) {

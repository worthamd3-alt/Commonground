...
function zb({
    ...
    Se = async S => {
      ...
      try {
        let {
          data: ppl,
          error: pplErr
        } = await F.from("profiles").select("id, active");
        if (pplErr) return "Couldn't send: " + pplErr.message;
        let to = (ppl || []).filter(row => row.active !== !1).map(row => row.id);
        if (!to.length) return "Nobody to notify";
        // Diagnostic: separates "my recipient list is wrong" from "notify can't
        // find the devices". Strip once the merch push is confirmed working.
        let meIn = e && e.id ? to.indexOf(e.id) !== -1 : !1,
          mySubs = await F.from("push_subscriptions").select("endpoint"),
          diag = to.length + " recipients, you " + (meIn ? "included" : "NOT included") + ", " + ((mySubs.data || []).length) + " own device rows visible \u2014 ";
        let {
          data: res,
          error: fnErr
          ...
          body: {
            kind: "announcements",
            to: to,
            title: "SOG Merch is open",
            body: [cat && cat.title, "Ordering is open", cat && cat.deadline].filter(Boolean).join(" \xB7 "),
            tag: "merch-open",
            url: "/Commonground/app-live.html"
          }
        });
        if (fnErr) return "Couldn't send: " + fnErr.message;
        if (res && res.error) return "Couldn't send: " + res.error;
        let n = res && res.sent || 0;
        return diag + (n ? "notified " + n + (n === 1 ? " device" : " devices") : "reply: " + JSON.stringify(res))
      } catch (ex) {
        return "Couldn't send: " + String(ex.message || ex)
      }
  ...
  let ae = async S => {
    ...
    let {
      ...
      pinned_until: new Date(Date.now() + 3 * 864e5).toISOString()
    });
